...
def plan_curved_trajectory(target_position, frame: int=0):
    """
    ...
    while not rospy.is_shutdown():
        try:
            ...
            trans = tfBuffer.lookup_transform(target, source, rospy.Time())## TODO: apply a lookup transform between our world frame and turtlebot frame
            print(trans)
            break
        except:
            pass
    # x1 and y1 are in robot frame, we 
    x1, y1 = trans.transform.translation.x, trans.transform.translation.y
    (roll, pitch, yaw) = tf.transformations.euler_from_quaternion(
        [trans.transform.rotation.x, trans.transform.rotation.y,
            trans.transform.rotation.z, trans.transform.rotation.w])
    
    DCM = np.array([[np.cos(yaw), -np.sin(yaw)],[np.sin(yaw), np.cos(yaw)]])
    
    if frame==0:
        target_odom = DCM@target_position
        x2 = target_odom[0] + x1
        y2 = target_odom[1] + y1 ## TODO: how would you get x2 from our target position? remember this is relative to x1 
    else:
        target_odom = target_position
        x2 = target_odom[0]
        y2 = target_odom[1]  ## TODO: how would you get x2 from our target position? remember this is relative to x1 
    print("error2")
